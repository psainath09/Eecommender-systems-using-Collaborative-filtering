...
# read the data for reader scale
...
from surprise.model_selection import search
from collections import defaultdict
#SVD
svd_results=[]
svd_algo = SVD(n_factors=100,n_epochs=6)
svd_algo.fit(train)
svd_predictions= svd_algo.test(test)
svd_results.append(accuracy.mae(svd_predictions))
svd_results.append(accuracy.rmse(svd_predictions,verbose=True))

# this function has been taken from surprise package documentation
...
precisions, recalls = precision_recall_at_k(svd_predictions, k=3, threshold=0.5)
print(sum(prec for prec in precisions.values()) / len(precisions))
print(sum(rec for rec in recalls.values()) / len(recalls))

svd_precision=[]
svd_precision.append(sum(prec for prec in precisions.values()) / len(precisions))
svd_precision.append(sum(rec for rec in recalls.values()) / len(recalls))
